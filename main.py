...
def run_explanation(args):
    if args.explainer == 'xpathExplainer':
        ...
        explanation, score_dict, scores_dict = model.get_explanation(data_df, explainer, None, step_size=100,
                                                                     top_k=args.top_k)

        relative_stocks_dict = select_explanation(args, explanation)
        relative_stocks_dict = select_top_k_related_stock(relative_stocks_dict,
                                                          k=args.top_k)
...
def select_top_k_related_stock(relative_stocks_dict, k=3):
    ...
    for d, s in relative_stocks_dict.items():
        ...
        for os, rs in s.items():
            ...
            if len(rs) == 0:
                stock_dict[os] = {}
            sorted_rs = sorted(rs, key=lambda x: x[1], reverse=True)[:k]
...
if __name__ == '__main__':
    args = parse_args()
    args.start_date = '2021-01-17'
    args.end_date = '2021-01-26'
    args.explainer = 'xpathExplainer'
    args.stock_list = ['SH600018']
    args.date_list = ['2021-01-18']
    s_t = time.time()
